...
import torch
from sklearn.metrics import f1_score, mean_squared_error, mean_absolute_error, recall_score, roc_auc_score
...
def evaluate_node_classification(predicts: torch.Tensor, labels: torch.Tensor):
    ...
    auc = roc_auc_score(labels, predicts.cpu().numpy()[:,1].tolist())
    macro_rec = recall_score(y_true=labels, y_pred=predictions, average='macro')
    macro_f1 = f1_score(y_true=labels, y_pred=predictions, average='macro')

    return accuracy, macro_f1, auc, macro_rec

def evaluate_p(predicts: torch.Tensor, labels: torch.Tensor):
    """
    get evaluation metrics for node classification, calculate accuracy and macro_f1 metrics
    :param predicts: Tensor, shape (N, )
    :param labels: Tensor, shape (N, )
    ...
    return accuracy, macro_f1, auc, macro_rec, labels, predictions
