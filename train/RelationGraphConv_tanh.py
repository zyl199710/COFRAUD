...
class RelationGraphConv(nn.Module):
    ...
    def forward(self, graph: dgl.DGLHeteroGraph, feat: tuple, dst_node_transformation_weight: nn.Parameter,
        ...
        # compute softmax
        #g = torch.tanh(graph.edata.pop('e'))
        #graph.edata['a'] = graph.apply_edges(g * graph.srcdata['d'] * graph.dstdata.src['d'])
    ...
    def edge_applying(self, edges):
        a = edges.src['e_src'] + edges.dst['e_dst']
        g = torch.tanh(a).squeeze()
        e = torch.mul(g , edges.src['d'] , )
        e = torch.mul(e, edges.dst['d'])
        return {'a':e.unsqueeze(dim = 2)}