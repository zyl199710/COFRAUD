...
class COFRAUD_Layer(nn.Module):
    def __init__(self, graph: dgl.DGLHeteroGraph, input_dim: int, hidden_dim: int, relation_input_dim: int,
                 ...
                 residual: bool = True, dataset: str = 'yelp', norm: bool = False):
        """
        ...
        """
        super(COFRAUD_Layer, self).__init__()
    ...
    def forward(self, graph: dgl.DGLHeteroGraph, relation_target_node_features: dict, relation_embedding: dict):
        ...
        if self.norm:
            for srctype, etype, dsttype in output_features_dict:
                output_features[(srctype, etype, dsttype)] = self.layer_norm[dsttype](output_features_dict[(srctype, etype, dsttype)])
        ...
        return output_features, relation_embedding_dict


class COFRAUD(nn.Module):
    def __init__(self, graph: dgl.DGLHeteroGraph, input_dim_dict: dict, hidden_dim: int, relation_input_dim: int,
                 ...
                 dropout: float = 0.2, negative_slope: float = 0.2, residual: bool = True, dataset: str = 'yelp', norm: bool = False):
        """
        ...
        self.layers.append(
            COFRAUD_Layer(graph, hidden_dim * n_heads, hidden_dim, relation_input_dim, relation_hidden_dim, n_heads,
                         dropout, negative_slope, residual, dataset,norm))
        for _ in range(1, self.num_layers):
            self.layers.append(COFRAUD_Layer(graph, hidden_dim * n_heads, hidden_dim, relation_hidden_dim * n_heads,
                                            relation_hidden_dim, n_heads, dropout, negative_slope, residual, dataset,norm))
